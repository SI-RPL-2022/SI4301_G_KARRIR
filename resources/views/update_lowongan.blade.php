        ...
        <div class="container pb-5">
            <div class="row">
                ...
                <div class="col-md-7 p-5 bg-white form-login">
                    ...
                    <form action="/provider/lowongan/update/{{$lowongan->id}}" method="POST">
                        @csrf
                        <div class="row mb-3">
                            <div class="col">
                        ...
                        <div class="row mb-3">
                            <div class="col">
                                <select class="form-select py-3 px-4" name="lokasi">
                                    ...
                                    <option @if ($lowongan->lokasi == 'Bandung') selected @endif>Bandung</option>
                                    <option @if ($lowongan->lokasi == 'Jakarta') selected @endif>Jakarta</option>
                                </select>
                            </div>
                            <div class="col">
                                <select class="form-select py-3 px-4" name="tipe">
                                    <option selected disabled hidden>Tipe</option>
                                    <option @if ($lowongan->tipe == 'Full-Time') selected @endif>Full-Time</option>
                                    <option @if ($lowongan->tipe == 'Part-Time') selected @endif>Part-Time</option>
                                </select>
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-md-3">
                                <select name="duit" id="duit" class="form-select py-3 px-4">
                                    <option value="USD" @if ($lowongan->duit == 'USD')
                                        selected
                                    @endif>USD</option>
                                    <option value="IDR" @if ($lowongan->duit == 'IDR')
                                        selected
                                    @endif>IDR</option>
                                </select>
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col">
                                ...
                                <input type="date" name="regis_start" id="regis_start" class="form-control py-3 px-4" value="{{ $lowongan->regis_start }}">
                            </div>
                            <div class="col">
                                <label for="regis_end" class="form-label">Registrasi ditutup</label>
                                <input type="date" name="regis_end" id="regis_end" class="form-control py-3 px-4" value="{{ $lowongan->regis_end }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col">
                                <textarea name="deskripsi" id="" rows="5" class="form-control @error('deskripsi') is-invalid @enderror"
                                    placeholder="Deskripsikan Pekerjaan">{{ $lowongan->deskripsi }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
